...
class ExponentCalc(Scene):
    def construct(self):
        ...
        # SEC_GIVENS --------------------------------------------------
        law = MathTex(r"a^{-n}=\frac{1}{a^{n}}").move_to(right_box.get_left()+RIGHT*0.2)
        LayoutGuard.ensure_no_overlap(self, [law], right_box)
        self.play(Write(law))
        board.add_line(self, law)

        # SEC_WORK ----------------------------------------------------
        step1 = MathTex(r"=-5\cdot\frac{1}{6^{2}}\cdot3\cdot1\cdot(-\frac{1}{3^{2}})")
        ...
        step2 = MathTex(r"=-\frac{5}{36}\cdot3\cdot(-\frac{1}{9})")
        ...
        # SEC_RESULT --------------------------------------------------
        result = MathTex(r"\frac{5}{108}").scale(1.5)
        LayoutGuard.ensure_no_overlap(self, [result], right_box)
        self.play(board.animate.fade_out(), FadeIn(result))
        box = SurroundingRectangle(result, buff=0.2)
