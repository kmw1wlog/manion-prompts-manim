...
class WorkRateScene(Scene):
    def construct(self):
        left_box, right_box = reserve_panels(self)
        board = RollingBoard(width=right_box.width * 0.95, max_lines=3)

        # SEC_PROBLEM
        problem = Text("A:10h, B:16h, 5h together", font_size=28)
        problem.move_to(right_box.get_left() + RIGHT * 0.3)
        LayoutGuard.ensure_no_overlap(self, [problem], right_box)
        self.play(Write(problem))
        board.add_line(self, problem)

        # left panel visual
        total = Rectangle(width=left_box.width * 0.8, height=0.4)
        total.move_to(left_box.get_center())
        together_bar = Rectangle(
            width=total.width * 13 / 16,
            height=0.4,
            fill_color=BLUE,
            fill_opacity=0.5,
        ).align_to(total, LEFT)
        solo_bar = Rectangle(
            width=total.width * 3 / 16,
            height=0.4,
            fill_color=GREEN,
            fill_opacity=0.5,
        ).next_to(together_bar, RIGHT, buff=0)
        bars = VGroup(total, together_bar, solo_bar)
        LayoutGuard.ensure_no_overlap(self, [bars], left_box)
        self.play(Create(total), FadeIn(together_bar), FadeIn(solo_bar))

        # SEC_GIVENS
        rate_line = MathTex(r"A: \frac{1}{10},\ B: \frac{1}{16}")
        rate_line.move_to(right_box.get_left() + RIGHT * 0.3)
        LayoutGuard.ensure_no_overlap(self, [rate_line], right_box)
        self.play(Write(rate_line))
        board.add_line(self, rate_line)

        # SEC_WORK
        work_done = MathTex(r"5\left(\frac{1}{10}+\frac{1}{16}\right)=\frac{13}{16}")
        work_done.move_to(right_box.get_left() + RIGHT * 0.3)
        LayoutGuard.ensure_no_overlap(self, [work_done], right_box)
        self.play(Write(work_done))
        board.add_line(self, work_done)

        remain = MathTex(r"1-\frac{13}{16}=\frac{3}{16}")
        remain.move_to(right_box.get_left() + RIGHT * 0.3)
        LayoutGuard.ensure_no_overlap(self, [remain], right_box)
        self.play(Write(remain))
        board.add_line(self, remain)

        solo_time = MathTex(r"\frac{\frac{3}{16}}{\frac{1}{16}}=3")
        solo_time.move_to(right_box.get_left() + RIGHT * 0.3)
        LayoutGuard.ensure_no_overlap(self, [solo_time], right_box)
        self.play(Write(solo_time))
        board.add_line(self, solo_time)

        # SEC_RESULT
        answer = MathTex(r"B:3\text{h solo}").scale(1.2)
        answer.move_to(right_box.get_center())
